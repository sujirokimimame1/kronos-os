...
exports.getRelatorios = async (req, res) => {
  try {
    ...
    console.log('🔍 Query relatórios:', query);
    console.log('📋 Parâmetros:', params);

    db.all(query, params, (err, rows) => {
      if (err) {
        console.error('❌ Erro no banco (relatórios):', err);
      ...
      console.log(`✅ ${rows.length} ordens de serviço encontradas para relatório`);
      ...
      chamados.forEach(os => {
        if (os.status === 'Finalizado' && os.data_fechamento && os.setor_destino) {
          ...
          if (
            tempo !== null &&
            (os.setor_destino === 'TI' || os.setor_destino === 'Manutenção')
          ) {
  ...
  try {
    ...
    db.all(query, [], (err, rows) => {
      ...
      const relatorioSetores = rows.map(row => ({
        ...
        taxaSucesso: Number(row.total || 0) > 0
          ? parseFloat(((Number(row.finalizadas || 0) / Number(row.total || 0)) * 100).toFixed(1))
          : 0,
        tempoMedio: '24h'
      }));
...
function construirIntervaloDatas(dataInicio, dataFim) {
  ...
  if (dataInicio && /^\d{4}-\d{2}-\d{2}$/.test(dataInicio)) {
    intervalo.inicio = `${dataInicio}T00:00:00.000Z`;
  }

  if (dataFim && /^\d{4}-\d{2}-\d{2}$/.test(dataFim)) {
    intervalo.fim = `${dataFim}T23:59:59.999Z`;
  }
  ...
  ordens.forEach(os => {
    if (os.data_abertura) {
      try {
        ...
        mensalCount[chave] = (mensalCount[chave] || 0) + 1;
      } catch (_) {}
    }
  });
